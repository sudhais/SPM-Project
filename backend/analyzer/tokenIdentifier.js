...
const Public = createToken({ name: "Public", pattern: /public/, categories: Access });
const Private = createToken({ name: "Private", pattern: /private/, categories: Access});
const Protected = createToken({ name: "Protected", pattern: /protected/, categories: Access});
const Static = createToken({ name: "Static", pattern: /static/ });

const CompoundOperator = createToken({ name: "CompoundOperator", pattern: Lexer.NA});
const And = createToken({ name: "And", pattern: /&&/, categories: CompoundOperator });
const Or = createToken({ name: "Or", pattern: /\|\|/, categories: CompoundOperator });

const Branch = createToken({ name: "Branch", pattern: Lexer.NA});
const If = createToken({ name: "If", pattern: /if/, categories: Branch });
const ElseIf = createToken({ name: "ElseIf", pattern: /else if/, categories: Branch });
...
const While = createToken({ name: "While", pattern: /while/, categories: Iterative });

const Type = createToken({ name: "Type", pattern: Lexer.NA});
const Int = createToken({ name: "Int", pattern: /int/, categories: Type });
const String = createToken({ name: "String", pattern: /String/, categories: Type });
const Boolean = createToken({ name: "Boolean", pattern: /boolean/, categories: Type });
...
const Double = createToken({ name: "Double", pattern: /double/, categories: Type });
...
const JavaTokens = [Access, Type, CompoundOperator, And, Or, Switch, Case, Iterative, For, While,Branch, If, ElseIf, Class, Static, Public, Private, Protected,  LeftBrace,Int, String, Boolean, Void, Float, Double, RightBrace, LeftParen, RightParen, Identifier];
...
class JavaParser extends CstParser {
    constructor() {
        ...
        this.RULE("compoundOperators", () => {
            this.CONSUME(CompoundOperator); // Assuming you want to capture the return type
        });
